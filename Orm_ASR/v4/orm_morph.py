...
def xsampa_to_latn(s):
    result = ''
    for c in s.split("~"):
        c = c.replace("t_>","x").replace("tS","ch").replace("tS_>","c").replace("k_>","q").replace("d_<","dh").replace("p_>","ph").replace("?","'").replace("S","sh").replace("J","ny").replace("j","y").replace("dZ","j")
        if c[-1:] == ':':
            c = c[:-1] + c[:-1]
        result = result + c
    return result
