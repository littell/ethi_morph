...
g2p = epitran.Epitran("tir-Ethi-red").transliterate 
...
g2pp = epitran.Epitran("tir-Ethi-red").transliterate 
...
xsampa_to_ipa = xsampa.XSampa().convert
def from_xsampa(s):
    s = s.replace("~","")
    s = xsampa_to_ipa(s)
    if s[:1] in ["a","e","i","o","u","ə","ɨ"]:
        s = "ʔ"+s
    return s
...
def fullparse(word, top=3, guess=True): 
    ...
    if ipa in l1_to_l2:    # whole word form found in dict file somewhere,
